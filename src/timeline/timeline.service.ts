...
export class TimelineService {
  ...
  private async fetchAllCached() {
    ...
    const entries = await this.prisma.timelineEntry.findMany({ orderBy: { order: 'asc' } })
    await this.cache.set(CACHE_KEY, entries, CACHE_TTL)
    return entries
  }

  async create(input: CreateTimelineEntryInput) {
    const result = await this.prisma.timelineEntry.create({ data: input })
  ...
  async update(id: string, input: UpdateTimelineEntryInput) {
    ...
    const result = await this.prisma.timelineEntry.update({ where: { id }, data: input })
    await this.cache.del(CACHE_KEY)
    return result
  }
